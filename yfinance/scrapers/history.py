...
class PriceHistory:
    ...
    def history(self, period=None, interval="1d",
        ...
        # Fix for https://github.com/ranaroussi/yfinance/issues/2676
        # Handle cases where chart payload is null or missing 'result'
        if data is None or data.get('chart') is None or data['chart'].get('result') is None:
            logger.warning(f"{self.ticker}: Yahoo Finance returned null result")
            return utils.empty_df()
        ...
        if correct_columns_individually:
            ...
            for j in range(len(OHLC)):
                ...
                if appears_suspended and (idx_latest_active is not None and idx_latest_active >= idx_first_f):
                    ...
                    ranges_after = map_signals_to_ranges(f_revj[idx_rev_latest_active:], f_rev_up[idx_rev_latest_active:], f_rev_down[idx_rev_latest_active:])
